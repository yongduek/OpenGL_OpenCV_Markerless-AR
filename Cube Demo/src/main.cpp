...
// VideoCapture cap(0);
VideoCapture cap("./videocaptured.avi");
...
void findAndComputePose(cv::Mat &img_scene)
{
	...
	// cv::cvtColor(img_scene, img_surf_input, cv::COLOR_BGR2GRAY);
...
void buildProjectionMatrix(float nearp=0.01, float farp=1000) {
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// else std::cout << frame.size() << std::endl;
